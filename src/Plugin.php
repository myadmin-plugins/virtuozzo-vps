...
class Plugin {
	...
	public static function getActivate(GenericEvent $event) {
		...
		if ($event['category'] == SERVICE_TYPES_FANTASTICO) {
			...
			activate_virtuozzo($serviceClass->get_ip(), $event['field1']);
	...
	public static function getChangeIp(GenericEvent $event) {
		if ($event['category'] == SERVICE_TYPES_FANTASTICO) {
			...
			$result = $virtuozzo->editIp($serviceClass->get_ip(), $event['newip']);
			if (isset($result['faultcode'])) {
				myadmin_log(self::$module, 'error', 'Virtuozzo editIp('.$serviceClass->get_ip().', '.$event['newip'].') returned Fault '.$result['faultcode'].': '.$result['fault'], __LINE__, __FILE__);
				$event['status'] = 'error';
